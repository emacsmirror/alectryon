...
while [ $# -gt 0 ]; do
  case "$1" in
    ...
    --vsrocq-version)    vsrocq_version="$2"; shift 2 ;;
...
if [ -n "$rocq_version" ]; then
  ...
  case "$vsrocq_version" in
    latest) ;; # No pin: latest tagged release from rocq-released.
    dev)    opam pin add -n vsrocq-language-server.dev https://github.com/rocq-prover/vsrocq.git --subpath=language-server ;;
    *)      opam pin add -n vsrocq-language-server "$vsrocq_version" ;;
  esac
